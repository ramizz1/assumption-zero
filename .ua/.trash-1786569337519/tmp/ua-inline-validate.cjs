...
try {
  ...
  graph.layers.forEach(layer => {
    for (const field of ['id', 'name', 'description', 'nodeIds']) if (layer[field] == null) issues.push(`Layer missing '${field}'`);
  ...
  graph.tour.forEach((step, i) => {
    for (const field of ['order', 'title', 'description', 'nodeIds']) if (step[field] == null) issues.push(`Tour step[${i}] missing '${field}'`);
