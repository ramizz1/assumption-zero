...
def generate_queries(idea: IdeaInput) -> list[dict[str, str]]:
    ...
    queries.append(
        {"query": f"{customer[:30]} {problem[:35]} software vendors", "type": "competitor"}
    )
    queries.append(
        {"query": f"{problem[:35]} software alternatives comparison", "type": "competitor"}
    )
    queries.append(
        {"query": f"how {customer[:30]} currently solve {problem[:30]}", "type": "manual_workflow"}
    )

    valid_comps = [
        c.strip()
        for c in known_comps.split(",")
    ...
    queries.append(
        {"query": f"{customer[:30]} pain points {problem[:30]} {geography}", "type": "complaint"}
    )
    ...
    if industry:
        queries.append(
            {"query": f"{industry} market size {geography} {current_year}", "type": "demand"}
        )
    ...
    queries.append(
        {"query": f"how {customer[:30]} manually does {problem[:30]}", "type": "manual_workflow"}
    )

    # ── Market direction ──────────────────────────────────────────
    queries.append(
        {"query": f"{problem[:40]} industry trends {current_year}", "type": "market_direction"}
    )
    if industry:
        queries.append(
            {
                "query": f"{industry} market trends {geography} {current_year}",
    ...
    if geography and geography != "global":
        ...
        queries.append(
            {"query": f"{customer[:30]} business statistics {geography}", "type": "geographic"}
        )
        queries.append(
            {
        ...
        if language:
            queries.append(
                {"query": f"{problem[:35]} {geography} {language}", "type": "geographic"}
            )

    # ── Regulatory concerns ───────────────────────────────────────
    queries.append(
        {
            "query": f"{industry or problem[:25]} regulations licenses tax {geography}",
            "type": "regulatory",
        }
    )
    queries.append(
        {"query": f"{problem[:30]} data privacy compliance regulations", "type": "regulatory"}
    )
    if constraints:
        queries.append(
            {
                "query": f"{industry or problem[:25]} {constraints} regulations {geography}",
                "type": "regulatory",
            }
        )
    ...
    queries.append(
        {
            "query": f"{industry or customer[:25]} associations directories events {geography}",
            "type": "distribution",
        }
    )
    if channels:
        queries.append(
            {"query": f"{channels} acquisition benchmarks {customer[:30]}", "type": "distribution"}
        )
