...
def _compute_scores(
    ...
    # ── Demand signals ────────────────────────────────────────────
    demand_score = min(
        100.0, demand_count * 18 + _avg_relevance(evidence, EvidenceType.DEMAND) * 30
    )
...
def _findings(
    perspective: PerspectiveName, evidence: list[EvidenceItem], idea: IdeaInput
) -> list[str]:
    findings = []
    if not evidence:
        findings.append(
            "No research evidence collected — configure research providers for a real analysis."
        )
        return findings
    ...
    if competitors:
        ...
        findings.append(
            f"{len(competitors)} direct or indirect competitor(s) found in research {ids}."
        )
    if demand:
    ...
    if not findings:
        findings.append(
            "Configure an AI provider (Gemini, Ollama, or OpenAI-compatible) "
    ...
    async def analyze_perspective(
        ...
        if perspective_name == PerspectiveName.MARKET_ANALYST:
            summary = (
                ...
                f"{len({e.source_name for e in evidence})} live search sources. "
            ...
            findings = [
                ...
                "SOM = Realistic first-year capture; requires acquiring first 1,000 paying accounts to establish baseline",
                ...
                "Revenue stream diversification opportunity: tiered pricing, API access, and enterprise plans should be modelled in Year 1",
            ]
            risks = [
            ...
            demand_local = [
                ...
                in (EvidenceType.DEMAND, EvidenceType.GEOGRAPHIC, EvidenceType.COMPLAINT)
            ]
            pricing_local = [
                item for item in regional if item.evidence_type == EvidenceType.PRICING
            ]
            regulatory_local = [
                item for item in regional if item.evidence_type == EvidenceType.REGULATORY
            ]
            distribution_local = [
                item for item in regional if item.evidence_type == EvidenceType.DISTRIBUTION
            ]
            summary = (
                f"Regional evidence baseline for {idea.name} in {loc}: {len(regional)} local items across "
                f"{len({item.source_name for item in regional})} sources. Global evidence is excluded from local demand claims."
            )
            findings = [
            ...
            findings = [
                ...
                "Lock-in mechanism required: proprietary data, integrations, or network effects must be built from Day 1",
                "§UNIT ECONOMICS & CAC/LTV§",
                "Customer acquisition cost (CAC): paid channel CAC must not exceed 20% of Year 1 LTV to be sustainable",
                ...
                "Risk #2 — Timing: Market may not be ready, or a larger player may enter with superior resources",
            ...
            risks = [
                ...
                "Paid acquisition CAC risks exceeding customer LTV in early launch phase.",
            ...
            opportunities = [
                ...
                "Proprietary workflow features create defensible retention moat.",
            ]
            ...
            pain_items = [
                ...
                for item in evidence
                if item.evidence_type
                in (EvidenceType.DEMAND, EvidenceType.COMPLAINT, EvidenceType.MANUAL_WORKFLOW)
            ]
            summary = (
            ...
            findings = [
                ...
                "Month 1: Scope and ship MVP — core value loop only; no secondary features; target: 10 beta users",
                "Month 2: Gather feedback from first 10 users; validate willingness to pay; onboard first 100 users",
                f"Month 3: Launch paid tier ({price_model}); target 3-5 paying accounts; instrument key retention metrics",
                "§TECH STACK & ARCHITECTURE§",
                "Recommended stack: Next.js / React frontend, FastAPI or Node.js backend, PostgreSQL, deployed on Vercel + Railway",
                "Estimated infra cost at launch: $50–80/mo; scales to $200-400/mo at 1,000 active users",
                "AI integration: use OpenRouter or OpenAI API; budget $50-100/mo for AI calls at early scale",
                ...
                "Risk mitigation: build in staged rollout, feature flags, and user feedback loop before scaling marketing spend",
            ]
            risks = [
                "Over-scoping MVP before core value is validated risks delayed launch and wasted runway.",
                "Third-party API costs can spike unexpectedly; set hard budget limits and alert thresholds from Day 1.",
            ]
            opportunities = [
                f"Focus Phase 1 on the core job-to-be-done for {idea.target_customer} only.",
                "Modern cloud stack enables lean infrastructure at <$80/mo initially.",
            ]
